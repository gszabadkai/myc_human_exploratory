...
| `E30` | `scripts/E30_mitoribosome_and_nadh.R`, `docs/2026-09-12_e30_mitoribosome_and_nadh.md` - **a second comparator for the gap, which FAILS the estimator ladder identically to E28's.** Two comparators with the same numerator, 2 of 4 estimators each, copy number disagreeing with both: that turns E28's inference into a demonstration that the failure is a property of the rank-gap statistic. Its mouse windows DO clear zero, where E28's did not. Changes nothing above |
| `E29` | `scripts/E29_oxphos_neighbourhood.R`, `docs/2026-09-12_e29_oxphos_neighbourhood.md` - **a MAP, explicitly not a readout.** It finds that position matches between groups are coincidental (no pair of 28 exceeds its rho-matched null), that the off-diagonal pathways are scattered, and that no opposing programme is established. It also records that mitoPPS is NOT a closed composition, which no earlier note states. Changes nothing above |
| `E28` | `scripts/E28_gap_readout.R`, `docs/2026-09-11_e28_gap_readout.md` - **a post-hoc gap readout that DOES NOT SURVIVE its own tests.** Its protection claim failed, the mouse contrast is not resolvable at n=6, and the human difference is estimator-dependent with the copy-number split disagreeing in sign. It carries nothing on the human side and changes nothing above, because neither E25 nor E26 is built on it |
